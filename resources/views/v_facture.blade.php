...
<body>
    <div class="button">
        <a class="btn btn-info" href="{{ url('detail-talon/' . $package[0]->client->id . '/' . $package[0]->id_talon) }}">Retour</a>
    ...
    <div id="pdf" class="container" >
        ...
        <div class="invoice" >
            <div class="row">
                ...
                <div class="col-5">
                    @if($package[0]->statut == 1)
                        <h1 class="document-type display-4">FACTURE</h1>
                        <p class="text-right">Nº: {{$package[0]->n_police}}</p>
                    @endif
                    @if($package[0]->statut == 0)
            ...
            <div class="row">
                <div class="col-7">
                    <div class="logo-table mb-2">
                        <img   style="width:100px;height:auto"src="{{asset('/storage/image/logo/' .$package[0]->compagnie->logo);}}"   class="logo img-fluid2"/>
                    </div>                
                </div>
                <div class="col-5">
                    <p class="addressDriver text-right">
                        ...
                        <span >{{$package[0]->client->civilite}}.{{$package[0]->client->name}}</span><br/>
                        <span >{{$package[0]->client->email}}</span><br/>
                        <span >{{$package[0]->client->contact}}{{$package[0]->client->phone}}</span> <br/>
                        <span>{{$package[0]->client->adrss}}</span>
                    </p>
                </div>
            </div>
            
            <h6 >Fait à Dakar le: <span>{{$package[0]->date_etablissement}}</span>  
            </h6>
            <table class="table table-striped mb-0">
                ...
                <tr>
                    ...
                    <td>{{$package[0]->vin}}</td>
                    <td>{{$package[0]->nombre_place}}</td>
                    <td>{{$package[0]->force_fiscale}} CV</td>
                    <td>{{$package[0]->date_first_circ}}</td>
            ...
            <table class="table table-striped mb-0">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-right">@foreach($packages->produits as $item)
            ...
            <table class="table table-striped mb-0">
                ...
                <tbody>
                <tr>
                    <td>{{ number_format($package[0]->prime_net, 0, ',', ' ') }} F CFA</td>
                    <td>{{ number_format($package[0]->coup_police, 0, ',', ' ') }} F CFA</td>
                    <td>{{ number_format($package[0]->taxe, 0, ',', ' ') }} F CFA</td>
                    <td>{{ number_format($package[0]->fga, 0, ',', ' ') }} F CFA</td>
                    <td>{{ number_format($package[0]->prime_ttc, 0, ',', ' ') }} F CFA</td>

                </tr>
                </tbody>
            
            </table>
            <!-- <h4 style="text-align: center;">Conditions particulières</h4> -->
            <table class="table table-striped mb-0">
                <thead>
                <tr>
                ...
                <tr>
                    <td>
                    <b>Sécurité routière:</b> décès {{ number_format($package[0]->capital_dece, 0, ',', ' ') }} F CFA, invalidité {{ number_format($package[0]->capital_invalidite, 0, ',', ' ') }} F CFA, frais médicaux: {{ number_format($package[0]->frais_medicaux, 0, ',', ' ') }} F CFA<br>
                    @foreach($packages->produits as $item)
                    @if($item->garantie->nom_court=== "vol")
                        <b>Incendie et vol:</b> {{ number_format($package[0]->valeur_venale, 0, ',', ' ') }} F CFA <br>
                    @endif
                    @if($item->garantie->nom_court=== "ar")
                    <b>Avance sur recours:</b>{{ number_format($package[0]->capitale_assure, 0, ',', ' ') }} F CFA <br>@endif
                    @if($item->garantie->nom_court=== "ar")
                    <b>Tierce complète:</b> {{ number_format($package[0]->valeur_neuf, 0, ',', ' ') }} F CFA @endif @endforeach</td> 
                    <td>@foreach($packages->produits as $item)
                    @if($item->garantie->nom_court=== "br")
                    <b>Bris de glace:</b> {{ number_format($package[0]->franchise_bdg, 0, ',', ' ') }} F CFA<br>@endif
                    @if($item->garantie->nom_court=== "vol")
                    <b>Vol:</b> {{ number_format($package[0]->franchise_vol, 0, ',', ' ') }} F CFA<br>@endif
                    @if($item->garantie->nom_court=== "tcmp")
                    <b>Tierce complete:</b> {{ number_format($package[0]->franchise_tr, 0, ',', ' ') }} F CFA @endif @endforeach</td>
                </tr>
                </tbody>
            ...
            <div class="row">
                <div class="col-8">
                    <p class="conditions" >
                                Le présent contrat est conclu pour la période indiquée ci-dessous.<br/>
                                <b> du {{$package[0]->date_effet}} au {{$package[0]->date_echeance}} à 23h:59</b><br/>
                                Il cessera de plein droit<br/>
                ...
                <!-- <div class="col-4">
                    <table class="table table-sm text-right" >
                        <tr>
                            ...
                            <td class="text-right" >{{ number_format($package[0]->prime_net, 0, ',', ' ') }} F CFA</td>
                        </tr>
                        <tr>
                            <td>Coût de police</td>
                            <td class="text-right" >{{ number_format($package[0]->coup_police, 0, ',', ' ') }} F CFA</td>
                        </tr>
                        <tr>
                            <td>Taxes (14%)</td>
                            <td class="text-right" >{{ number_format($package[0]->taxe, 0, ',', ' ') }} F CFA</td>
                        </tr>
                        <tr>
                            <td>FGA</td>
                            <td class="text-right"  >{{ number_format($package[0]->fga, 0, ',', ' ') }} F CFA</td>
                        </tr>
                        <tr>
                            <td><strong>Primes TTC</strong></td>
...
<script>
     function SaveToPDF() {
        ...
        var clientName = '{{$package[0]->client->name}}';
        var clientN_police = '{{$package[0]->n_police}}';
        var options = {
