...
class CompanyController extends Controller
{
    ...
    */
            ...
            $custom_fields->each(function ($custom_field) use ($model) {
                try {
                    ...
                    if ($existing_custom_field->type === $custom_field['type']) {
                        $existing_custom_field->value = isset($custom_field['default']) ? (is_array($custom_field['default']) ? (in_array(json_decode($existing_custom_field->value)->choice, $custom_field['default']['choices'], true) ? json_encode([
    ...
    {
        // echo($request->custom_fields);exit;
        // echo($request->custom_fields);exit;
        ...
        // ]);
        // echo($request);exit;
        $custom_fields_metadata = collect([]);
        if (isset($request->custom_fields)) {
            foreach ($request->custom_fields as $custom_field) {
                ...
                if ($custom_field['type'] === 'select') {
                    $custom_field['default']['choices'] = explode(',', str_replace([', ', "\r\n", "\n"], ',', $custom_field['default']['choices']));
        ...
        if ($request->hasFile('logocompany')) {
            $logo_filename = str_random(7).'-logo.'.str_replace('jpeg', 'jpg', $request->file('logocompany')->guessExtension());

            try {
                $request->file('logocompany')->move(storage_path('app/public/image/logo/company/'), $logo_filename);
            } catch (FileException $e) {
                return redirect()->back()->withErrors([
                    trans('compagnie.message.errors.file', [
                        'filename'  => $request->file('logo')->getClientOriginalName(),
                    ]),
                ]);
            }
        }	
        // var_dump( $request->file('logocompany'));exit;
        ...
        $company->logo = $logo_filename ?: null;
